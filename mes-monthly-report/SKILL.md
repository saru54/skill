...
## 3. 🧠 动态数据解析、清洗与业务维度自动归类引擎 (Dynamic Parsing Engine)

> [!IMPORTANT]
> **严禁硬编码**：月报内容绝非硬编码模板，而是**根据任意输入的工作记录 JSON 动态分析提取、语义清洗并自适应归类**。无论是任意月份（8月、9月、10月）还是任意开发工程师，工具均能自动提取其真实工作并映射至对应幻灯片。

### 3.1 动态字段解析与清洗逻辑
对每一条日常工作记录（`rows[].list[]`）：
1. **任务描述提取**：优先读取 `nonCore.任务描述` 或 `nonCore.学习目标`；若无则提取 `item.content`。
2. **文本清洗与去噪**：自动剔除代码库 URL（`https://...`）、多余回车换行符（转换为规范中文逗号分句），去重相同工作。
3. **状态动态判定**：
   - 若 `item.progress === "100"`（或 `>= 100`）且非明确研究中词汇，标记为企业红字 **`完成`**；
   - 若 `item.progress < 100`，或文本明确包含“学习自学习”、“探索使用”等延续性研究，动态标记为企业红字 **`进行中`**。

### 3.2 业务维度自动分流引擎

```mermaid
graph TD
    Raw[任意月份工作记录 JSON] --> Parser[动态清洗: 提取描述/状态/姓名/组别]
    Parser --> Filter{时间范围过滤器<br>--month / --range}
    Filter --> CleanTasks[去重有效任务集]
    
    CleanTasks --> KeywordRouter{关键词与业务属性动态路由}
    KeywordRouter -->|MES/WinForms/质量/半成品/物料/报表/权限| S4[Slide 4: MES业务系统开发]
    KeywordRouter -->|PDA/MUI/Vue/ASHX/Skill/Agent/Kepware/培训| S5[Slide 5: PDA与技术攻关/AI建设]
    
    S4 & S5 --> SynthS3[动态合成 Slide 3: 已完成核心工作总括]
    S4 & S5 --> SynthS7[动态提取 Slide 7: 5大关键里程碑达成率]
    S4 & S5 --> SynthS9[动态生成 Slide 9: 延续进行中任务与后续规划]
```

### 3.3 幻灯片内容动态生成与上限截取
- **Slide 4（MES 核心业务）**：动态聚合所有与 MES 管理端、生产工艺、物料流转相关的条目，上限截取 7 条，单条字号 11.5pt，自适应行间距 1.25x。
- **Slide 5（PDA、技术攻关与培训）**：动态聚合移动端、AI 工具链、工控对接与企业培训相关条目，上限截取 9 条，单条字号 11pt，自适应行间距 1.15x。
- **Slide 3（日常工作总览）**：动态提取 Slide 4 与 Slide 5 的前 3 项核心成果，自动合成：`已完成：[任务A]、[任务B]、[任务C]。未完成：无`。
- **Slide 7（上月计划达成率）**：动态提取 4~5 项最具代表性的完成成果，自动装配 Wingdings 绿色圆点（`l`）与打勾（`ü`）。
- **Slide 9（工作计划）**：自动提取处于 `进行中` 的攻关任务，并与智能制造连续性规划联动生成 4 项行动项。
