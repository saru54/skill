...
### 📊 配方 D：智能制造/MES 工程师月报与周期性汇报生成
适用于根据日常工作记录 JSON、系统开发日志自动编制公司级汇报 PPT：

1. **Phase 1 (数据提炼)**：加载 `mes-monthly-report`，解析输入工作记录 JSON，按 4 大核心业务维度（MES管理端开发、移动端PDA架构、AI工作流效能、工业互联数采）归纳合并提炼技术成果。
2. **Phase 2 (OpenXML无损注入)**：解包企业月报 PPT 模板，将汇报人信息、各模块工作明细、计划达成率（Wingdings 图标）与下月计划（SmartArt 节点）精准注入底层 XML 并自适应行间距与字号。
3. **Phase 3 (封包与质检)**：重新打包生成目标 PPTX，并调用 `scripts/export_ppt_slides.ps1` 导出全量 JPG 幻灯片执行防溢出与视觉核查，最终归档交付。
